...
def protected_resource():
    ...
    try:
        ...
        print(f"Logged user is: {username}")
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT jwt_token FROM users WHERE username=?", (username,))
        row = cursor.fetchone()
        conn.close()
        
        if not row or row[0] != token:
            return jsonify({"message": "Token revoked or invalid"}), 401
